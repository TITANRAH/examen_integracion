...
namespace TranslatorXml
{
    ...
    class Program
    {
        ...
        {
            var transformer = new XmlToCanonicalTransformer();
            ...
            using (var producer = new MsmqProducer(QUEUE_DESTINO))
            {
                ...
                {
                    string metadatos;
                    var pagoXml = consumer.LeerPago(out metadatos);
                    ...
                    Console.WriteLine(string.Format("Leído: {0} [Metadatos: {1}]", pagoXml, metadatos));

                    var pagoCanonical = transformer.Transform(pagoXml, metadatos);
        ...
        private static void MostrarBanner()
        {
            Console.Clear();
            Console.WriteLine("════════════════════════════════════════");
            Console.WriteLine("  Translator XML - Aukan Gym");
